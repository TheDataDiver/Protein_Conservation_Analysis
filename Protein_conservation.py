...
####### Creating output folder for the motifs
	os.mkdir('%s/Assignment2_%s/motifsofplotted' % (home, moment))
####### Creating a list of all plotted Protein accessions, from the updated dataframe
	plottedaccessionlist = df['Prot Accession'].tolist()
####### Creating output folder for fasta files which are to be used for motif
	os.mkdir('%s/Assignment2_%s/motifsofplotted/fastafiles' % (home, moment))
####### Creating a file containing plotted protein sequence accessions, so that pullseq can be performed on it later
	for elem in plottedaccessionlist:
		with open('%s/Assignment2_%s/motifsofplotted/fastafiles/%s.txt' % (home, moment, elem), 'w') as f:
			f.write('%s\n' % (elem))
		f.close()
####### For loop to pullseq each sequence into its own fasta file
	print('Now scanning the plotted proteins with motifs from the PROSITE database, to determine if any known motifs (domains) are associated with the subset of sequences in the dataset\n')
	for elem in plottedaccessionlist:
		subprocess.call("/localdisk/data/BPSM/Assignment2/pullseq -i '%s/Assignment2_%s/fastafiles/multialign.fasta' -n '%s/Assignment2_%s/motifsofplotted/fastafiles/%s.txt' > '%s/Assignment2_%s/motifsofplotted/fastafiles/%s.fa'" % (home, moment, home, moment, elem, home, moment, elem), shell=True)
		subprocess.call("patmatmotifs -sequence '%s/Assignment2_%s/motifsofplotted/fastafiles/%s.fa' -auto Y -outfile '%s/Assignment2_%s/motifsofplotted/fastafiles/%s_motif.txt'" % (home, moment, elem, home, moment, elem), shell=True)
####### Merging the motif files into a single file
	subprocess.call("find %s/Assignment2_%s/motifsofplotted/fastafiles -name '*motif.txt' | xargs -I {} cat {} > %s/Assignment2_%s/motifsofplotted/Plottedmotifs.txt" % (home, moment, home, moment), shell=True)
	print('Motif analysis is complete')
	print('\n====================================================================================')
####### Grepping all the lines with Hit count, this shows number of motifs per sequence
	motifs = subprocess.check_output("cat %s/Assignment2_%s/motifsofplotted/Plottedmotifs.txt | grep 'HitCount'" % (home, moment), shell=True)
	motif = motifs.decode("utf-8").strip()
####### creating list from the grepped line
	listofmotifs = motif.split()
	totalplotted = df.shape[0]
####### starts at the second index and keeps every 3rd element
	seqwithmotif = listofmotifs[2::3]
####### Total sequences that were plotted
	totalsequences = len(seqwithmotif)
####### Filters out all sequences with 0 hit counts from the list
	seqwithmotifs = [x for x in seqwithmotif if x != '0']
####### Gets length of the list, this is total number of sequences with motifs
	numberseqmotif = len(seqwithmotifs)
####### converts list to integers, allowing for addition and to count total number of motifs
	numberofmotifs = [int(i) for i in seqwithmotifs]
####### total motifs found, by summing the list of integers
	totalmotifsfound = sum(numberofmotifs)
####### getting list of uniq motifs found
	var = subprocess.check_output("cat '/localdisk/home/s1962117/Assignment2_08_44/motifsofplotted/Plottedmotifs.txt' | grep 'Motif =' | uniq", shell=True)
	vars = var.decode("utf-8").strip()
####### Printing file paths of important output files for the user, so he can access it for further in-depth analysis
	print('Motif analysis summary:\n\nTotal Sequences analysed/plotted: %s\nTotal number of motifs found: %s\nTotal number of Sequences with Motifs: %s\nMotifs that were found: %s' % (totalplotted, totalmotifsfound, numberseqmotif, vars))
	print('\nFor more in-depth analysis of the sequences and their identified motifs, please access the motif analysis output file. Path to this file will be displayed below')
	print('\n====================================================================================')
	print('Location/path of files that were generated by this script are as follows:')
	print('\nConservation Plot Image:\n\t%s/Assignment2_%s/conservationplot/ConservationPlot.svg' % (home, moment))
	print('\nPlotted Sequence dataframe:\n\t%s/Assignment2_%s/conservationplot/PlottedProteinDataFrame.txt' % (home, moment))
	print('\nMotif analysis output:\n\t%s/Assignment2_%s/motifsofplotted/Plottedmotifs.txt' % (home, moment))
	print('\n====================================================================================')
	print('\n\nThe script has come to an end, and so has the endless sleepless nights haha')
